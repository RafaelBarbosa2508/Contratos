...
from utils import reiniciar_aplicativo, formatar_moeda, formatar_chave, formatar_cnpj, placa_valida, limpar_valor, limpar_placa
...
def main():
    ...
    arquivos = st.file_uploader("Anexe os arquivos XML para começar", accept_multiple_files=True, type=["pdf", "xml"], key=f"uploader_principal_{st.session_state.uploader_key}")
...
# 1. Trava amarela: Aguarda o primeiro upload
    ...
    if xml_nfe_ok and not xml_cte_ok and not xml_mdf_ok:
        ...
        # 1. Entrada do CNPJ
        cnpj_input = st.text_input("DIGITE O CPF / CNPJ DO SUBCONTRATADO", value=st.session_state.get("transp_cnpj_last", ""), key="cnpj_input_field")
        # Limpamos a entrada para contar apenas os números
        cnpj_limpo = re.sub(r'\D', '', cnpj_input)

        # 2. LÓGICA DE LIMPEZA CRÍTICA: Se não tiver 14, nada abaixo existe
        if len(cnpj_limpo) != 14:
            ...
            # Limpamos o dicionário 'dados' para garantir que o IF do motorista falhe
            dados["transp_nome"] = ""
            dados["transp_end"] = ""
            ...
            if not cnpj_limpo:
                ...
                st.warning("⚠️ O CNPJ deve ter exatamente 14 dígitos.")
            
            # O st.stop() aqui garante que o bloco "Motorista e Veículo" NÃO seja lido
            st.stop()
        ...
        # 4. Lógica de Busca (Só chega aqui se tiver 14 dígitos e não for bloqueado)
        if cnpj_limpo != st.session_state.get("transp_cnpj_last", ""):
            with st.spinner("Buscando dados oficiais..."):
                ...
                if res:
                    ...
                    st.session_state.transp_cnpj_last = "" # Reseta para permitir nova busca
                    st.stop()
        ...
        # 6. Sincronização Final com o dicionário de dados
        ...
        if dados.get("transp_nome") and dados.get("transp_end"):
            ...
            # --- VALIDAÇÃO DA CARRETA 2 (DENTRO DO SEU BLOCO DE CÓDIGO) ---
            v_c2 = p_carreta2.strip()
            if v_c2:
                # 1. Dividimos a entrada caso haja uma barra "/"
                # Se não houver barra, o split devolve uma lista com um único item
                partes_placa = [p.strip() for p in v_c2.split('/')]
                
                placas_limpas_validadas = []

                for p in partes_placa:
                    if p: # Ignora se houver algo como "ABC1234 / "
                        if not placa_valida(p):
                            st.error(f"❌ A PLACA '{p}' na CARRETA 2 está em um formato inválido.")
                            st.stop()
                        # Se for válida, limpamos ela para padronizar o salvamento
                        placas_limpas_validadas.append(limpar_placa(p))
                
                # 2. Salvamos no dicionário com a formatação padrão "PLACA1 / PLACA2"
                dados["placa_carreta2"] = " / ".join(placas_limpas_validadas)
            else:
                dados["placa_carreta2"] = ""

            # =====================================================================

            # Lógica da Carreta 3 (Mantendo a compatibilidade com seu sistema de impressão)
            if p_carreta1 and dados["placa_carreta2"]:
                dados["placa_carreta3"] = f"{p_carreta1} / {dados['placa_carreta2']}"
            else:
                dados["placa_carreta3"] = p_carreta1 or dados["placa_carreta2"] or ""
            ...
            if dados.get("motorista") and dados.get("cpf_motorista") and dados.get("placa_cavalo"):
                ...
                with col_fin1:
                    ...
                    frete_edit = limpar_valor(frete_txt)
    ...
    # --- NOVO BLOCO: DADOS DO SUBCONTRATADO (TRANSPORTADOR) ---
    st.markdown("---")
    st.subheader("🚛 Dados do Subcontratado")

    # Tentamos pegar o CNPJ que veio do XML, se não houver, usamos o último digitado
    cnpj_sugerido = dados.get("transp_cnpj") if dados.get("transp_cnpj") else st.session_state.get("transp_cnpj_last", "")
    
    cnpj_input = st.text_input(
        "DIGITE O CPF / CNPJ DO SUBCONTRATADO", 
        value=cnpj_sugerido,
        key="cnpj_input_field"
    )
    cnpj_limpo = re.sub(r'\D', '', cnpj_input)

    # Validação de 14 dígitos e Limpeza em tempo real
    if len(cnpj_limpo) != 14:
        st.session_state.transp_nome = ""
        st.session_state.transp_end = ""
        st.session_state.transp_cnpj_last = ""
        ...
        if not cnpj_limpo:
            ...
            st.warning("⚠️ O CNPJ deve ter exatamente 14 dígitos.")
        st.stop() # TRAVA O FLUXO AQUI

    # Bloqueio de Filiais Próprias
    bloqueados = ["32368678000118", "32368678000207", "32368678000380", "32368678000460", "32368678000541", "32368678000622", "32368678000703"]
    if cnpj_limpo in bloqueados:
        st.error("🚫 **BLOQUEADO:** Este CNPJ pertence a uma das filiais da Rodo Amazônia.")
        st.stop()

    # Busca Automática se for um CNPJ novo
    if cnpj_limpo != st.session_state.get("transp_cnpj_last", ""):
        with st.spinner("Buscando dados oficiais..."):
            res = buscar_endereco_por_cnpj(cnpj_limpo)
            if res:
    ...
    # Exibição Visual dos dados encontrados (Apenas Leitura)
    col_sub1, col_sub2 = st.columns([1, 1])
    with col_sub1:
        st.text_input("RAZÃO SOCIAL", value=st.session_state.get("transp_nome", ""), disabled=True)
    with col_sub2:
    ...
    # Sincroniza com o dicionário de dados principal
    dados["transp_cnpj"] = formatar_cnpj(cnpj_limpo)
    dados["transp_nome"] = st.session_state.get("transp_nome", "")
    dados["transp_end"] = st.session_state.get("transp_end", "")

    # SÓ LIBERA O RESTANTE SE HOUVER NOME E ENDEREÇO
    if not dados.get("transp_nome") or not dados.get("transp_end"):
        st.stop()
